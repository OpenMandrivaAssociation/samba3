...
%if %build_vscan
%setup -q -a 8 -n samba-%{source_ver}
%else
%setup -q -n samba-%{source_ver}
%endif
# Version specific patches: current version
%if !%have_pversion
...
rm %{buildroot}/%{_datadir}/samba/bin/svf-notify
perl -pi -e 's,%{_datadir}/samba/bin,%{_datadir}/samba/scripts,g' %{buildroot}/%{_sysconfdir}/%{name}/smb.svf-*.conf
%endif
...
#        install -m644 examples/VFS/recycle/recycle.conf %{buildroot}%{_sysconfdir}/samba/
        ...
        install -m755 %{SOURCE24} %{buildroot}/%{_sbindir}/samba
...
%{_datadir}/pixmaps/%name/samba.ico
